...
def create_modules(module_defs, img_size, arc, deploy):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        elif mdef['type'] == 'RepvggBlock':
            if deploy==False:
                modules_repvgg = nn.ModuleList()
                bn = int(mdef['batch_normalize'])
                filters = int(mdef['filters'])
                kernel_size = int(mdef['size'])
                stride=int(mdef['stride'])
                pad = int(mdef['pad'])
                dense = conv_bn(in_channels=output_filters[-1],out_channels=filters,kernel_size=kernel_size,stride=stride,padding=pad)
                identy = nn.BatchNorm2d(num_features=filters) if output_filters[-1]==filters and stride==1 else None
                conv1x1 = conv_bn(in_channels=output_filters[-1],out_channels=filters,kernel_size=1,stride=stride,padding=0)
                modules_repvgg.append(dense)
                modules_repvgg.append(conv1x1)
                modules_repvgg.append(identy)
                if mdef['activation'] == 'relu':
                    modules_repvgg.append(nn.ReLU())
                module_list.extend(modules_repvgg)
                ...
                if mdef['activation'] == 'relu':  # TODO: activation study https://github.com/ultralytics/yolov3/issues/441
                    module_list.extend(nn.ModuleList().append(nn.ReLU()))
                output_filters.append(filters)
                continue
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default', deploy=False):
        #我的添加
        ...
        self.module_list, self.routs = create_modules(self.module_defs, img_size, arc, deploy=self.deploy)
    ...
    def forward(self, x, var=None):
        ...
        for mdef in self.module_defs:
            ...
            if mtype in ['convolutional', 'quantize_convolutional','upsample', 'maxpool']:
                ...
                # print(module)
                x = module(x)
            elif mtype == 'RepvggBlock':
                if self.deploy==False:
                    # print(i)
                    module=[self.module_list[i],self.module_list[i+1],self.module_list[i+2],self.module_list[i+3]]
                    if module[2] is None:
                        id_out=0
                    else:
                        id_out=module[2](x)
                    x = module[3](module[0](x) + module[1](x) + id_out)
                    # layer_outputs.append(x if int((i-1)/4+1) in self.routs else [])       ###
                    layer_outputs.append(x if int(i/4) in self.routs else [])
                    i = i + 4
                    continue
                else:
                    module = self.module_list[i]
                    ...
                    x = module(x)
                    layer_outputs.append(x if int(i/2) in self.routs else [])
                    i = i + 2
                    continue
            elif mtype == 'route':
                module = self.module_list[i]
            ...
            if self.deploy==False:
                layer_outputs.append(x if i+1-113+28 in self.routs else [])       ###
            else:
                layer_outputs.append(x if i-28 in self.routs else [])
            i = i + 1
